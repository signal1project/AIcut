...
const Timeline: React.FC = () => {
  const { tracks, playhead, duration, zoom, setPlayhead, addTrack } = useEditorStore();
  ...
  useEffect(() => {
    ...
    if (playheadPx > visibleEnd - 40 || playheadPx < visibleStart) {
      el.scrollLeft = Math.max(0, playheadPx - (el.clientWidth - LABEL_WIDTH) / 2);
    }
  }, [playhead, zoom]);
  ...
  return (
    <div className="flex flex-col h-full bg-[#0e0e11] select-none overflow-hidden">
      ...
      <div className="flex shrink-0">
        ...
        >
          <span className="text-[10px] text-[#5a5a66] uppercase tracking-wider font-medium">Timeline</span>
        </div>
        <div className="flex-1 overflow-hidden" onMouseDown={handleRulerMouseDown} style={{ cursor: 'text' }}>
          <TimelineRuler duration={Math.max(duration + 10, 30)} zoom={zoom} scrollLeft={scrollLeft} />
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto relative" onScroll={handleScroll}>
        ...
        <div className="flex flex-col" style={{ minWidth: LABEL_WIDTH + totalWidth }}>
          {tracks.map((track) => (
            <TimelineTrack key={track.id} track={track} zoom={zoom} totalWidth={totalWidth} labelWidth={LABEL_WIDTH} />
          ))}

          {/* Add-track row */}
          <div className="flex items-center gap-3 px-3 py-2.5 bg-[#0c0c0f] border-t border-[#1a1a1f]">
            <button onClick={() => addTrack('video')} className="add-track add-track--video">
              <Plus size={11} strokeWidth={2.5} /> <Film size={11} /> Video
            </button>
            <button onClick={() => addTrack('audio')} className="add-track add-track--audio">
              <Plus size={11} strokeWidth={2.5} /> <Music size={11} /> Audio
            </button>
            <button onClick={() => addTrack('caption')} className="add-track add-track--caption">
