...
const WAVE = Array.from(
  { length: 48 },
  (_, i) => 0.35 + 0.6 * Math.abs(Math.sin(i * 1.7) * Math.cos(i * 0.6)),
);
...
const TimelineClip: React.FC<Props> = ({ clip, zoom, trackLocked }) => {
  ...
  const effectiveDuration =
    (clip.duration - clip.trimStart - clip.trimEnd) / (clip.speed ?? 1);
      ...
      const onMove = (me: MouseEvent) => {
        ...
        if (trimRef.current.edge === 'left') {
          const newTrimStart = Math.max(
            0,
            Math.min(
              trimRef.current.origTrimStart + delta,
          ...
          const newTrimEnd = Math.max(
            0,
            Math.min(
              trimRef.current.origTrimEnd - delta,
