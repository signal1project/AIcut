...
const PropertiesPanel: React.FC = () => {
  ...
  const effectiveDuration =
    selectedClip.duration - selectedClip.trimStart - selectedClip.trimEnd;
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex items-center gap-2.5 px-3 py-3 border-b border-[#202027]">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-[#71717f]">
              {meta.label} · {effectiveDuration.toFixed(1)}s
            </p>
          </div>
        </div>
        ...
        <div className="p-3 space-y-4">
            ...
            <Field label="Volume">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 accent-[#4d7cff]"
                />
                <span className="text-[10px] text-ink-base w-9 text-right tabular-nums">
          ...
          {selectedClip.type === 'caption' && (
            <Field label="Caption text">
              <textarea
                ...
                onChange={(e) =>
                  updateClip(selectedClip.id, { captionText: e.target.value })
                }
            ...
            <Field label="Transitions">
              <div className="space-y-2.5">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-[10px] text-ink-base w-9 text-right tabular-nums">
                    {(selectedClip.fadeIn ?? 0).toFixed(1)}s
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-[#71717f] w-14 shrink-0">
                  ...
                  />
                  <span className="text-[10px] text-ink-base w-9 text-right tabular-nums">
                    {(selectedClip.fadeOut ?? 0).toFixed(1)}s
                  </span>
                </div>
              </div>
            </Field>
          ...
          {selectedClip.src && (
            <Field label="Source">
              <p className="text-[9px] text-[#4a4a55] break-all leading-relaxed font-mono">
                {selectedClip.src}
              </p>
            </Field>
          )}
